...
class Ocpp15JsonParser(override val ignoreValidationCodes: List<String> = emptyList()) :
    OcppJsonParser(Ocpp15JsonObjectMapper) {
        ...
        try {
            Actions.valueOf(action.uppercase()).let {
                javaClass.classLoader.loadClass(
                    "com.izivia.ocpp.core15.model.${it.lowercase()}.${it.camelCaseRequest()}"
                )
            }
        } catch (e: Exception) {
