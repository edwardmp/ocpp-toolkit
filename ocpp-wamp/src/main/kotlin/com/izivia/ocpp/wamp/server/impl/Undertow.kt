...
class Undertow(
    ...
    val wsConnectionCallbackSelectorByHeaders: (HttpServerExchange) -> Boolean
) : PolyServerConfig {
    override fun toServer(http: HttpHandler?, ws: WsHandler?, sse: SseHandler?): Http4kServer {
        ...
        val wsCallback: WebSocketProtocolHandshakeHandler? = ws?.let {
        ...
        val wsIgnoreCorruptCallback = ws?.let {
            WebSocketProtocolHandshakeHandler(
                listOf(
                    IgnoreCorruptMessageHybi13Handshake(wsSubprotocols, false)
                ),
                UndertowWebSocketCallBack(it)
            )
        }
        ...
        val handlerWithWs = predicate(
            ...
            predicate(
                { exch -> wsConnectionCallbackSelectorByHeaders(exch) },
                wsIgnoreCorruptCallback,
                wsCallback
            ),
            httpHandler
        )
